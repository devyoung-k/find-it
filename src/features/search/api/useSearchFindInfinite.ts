...
import { searchFoundItems } from '@/features/search/api/searchItems';
...
export const useSearchFindInfinite = (
  ...
  return useInfiniteQuery<AllData[], Error>({
    queryKey: ['searchFindResult', keyword, pageSize],
    initialPageParam: 0,
    enabled: options?.enabled,
    queryFn: async ({ pageParam }) =>
      searchFoundItems(keyword, typeof pageParam === 'number' ? pageParam : 0, pageSize),
